...
(function (IdValidatorMode) {
	...
	IdValidatorMode[(IdValidatorMode["removeAllProofs"] = 2)] =
		"removeAllProofs";
	IdValidatorMode[(IdValidatorMode["removeAllBuckets"] = 3)] =
		"removeAllBuckets";
...
class ManifestIdValidator {
	constructor(document) {
		...
		this.addBucketDefinition = (bucketDefinition) => {
			let bucketDefIndex = this.availableBuckets
		...
		this.addProofDefinition = (proofDefinition) => {
			let proofDefIndex = this.availableProofs
		...
		if (end) {
			...
			if (children) {
				...
				for (var token of flattenedChildren) {
					if (
						token instanceof
						TransactionManifestParser_1.BucketContext
					) {
						...
						token instanceof
						TransactionManifestParser_1.ProofContext
					) {
...
function flattenTree(tree) {
	...
	for (var token of tree) {
		if (
			token instanceof TransactionManifestParser_1.BucketContext ||
			token instanceof TransactionManifestParser_1.ProofContext
		) {
