...
async function activate(context) {
	 ...
	 */
	const defaultAccount = vscode.commands.registerCommand(
		...
		async () => {
			...
			if (currentDefaultAccount) {
				...
				let resimNewAccountResponse = await resimUtils.executeShell(
					"resim new-account"
				);
	 ...
	 */
	const newAccount = vscode.commands.registerCommand(
	...
	vscode.languages.registerDocumentFormattingEditProvider("rtm", {
		provideDocumentFormattingEdits(document) {
			...
			let charStream =
				antlr4ts_1.CharStreams.fromString(documentContents);
			let lexer = new TransactionManifestLexer_1.TransactionManifestLexer(
				charStream
			);
			let tokenStream = new antlr4ts_1.CommonTokenStream(lexer);
			let parser =
				new TransactionManifestParser_1.TransactionManifestParser(
					tokenStream
				);
			let tree = parser.manifest();
			const listener =
				new manifest_format_listener_1.ManifestFormatterListener();
	...
	vscode.languages.registerHoverProvider("rtm", {
		provideHover(document, position, token) {
			...
			let charStream = antlr4ts_1.CharStreams.fromString(
				document.getText()
			);
			let lexer = new TransactionManifestLexer_1.TransactionManifestLexer(
				charStream
			...
			if (hoveredOverToken) {
				...
				let tokenLength =
					hoveredOverToken.stopIndex - hoveredOverToken.startIndex;
				let range = new vscode.Range(
					hoveredOverToken.line - 1,
					hoveredOverToken.charPositionInLine,
					hoveredOverToken.line - 1,
					hoveredOverToken.charPositionInLine + tokenLength + 1
				);
				let tokenStringRepresentation = document.getText(range);
				hoverResponse = (0, hover_strings_1.identifierToHover)(
	...
	const getDiagnosticsWithLexerAndParserErrors = (document) => {
		...
		let tokenStream = new antlr4ts_1.CommonTokenStream(lexer);
		let parser = new TransactionManifestParser_1.TransactionManifestParser(
			tokenStream
		);
	...
	const getIdValidatorDiagnostics = (document) => {
		...
		const listener =
			new manifest_id_validator_listener_1.ManifestIdValidator(document);
		let tree = parser.manifest();
		// @ts-ignore
		ParseTreeWalker_1.ParseTreeWalker.DEFAULT.walk(listener, tree);
