...
suite("Addresses Diagnostics Provider", () => {
    test('Hex address is treated as invalid Bech32', () => {
        ...
        let diagnosticsProvider: AddressesDiagnosticProvider = new AddressesDiagnosticProvider(textDocument);

        // Act
        let diagnostics: Diagnostic[] = diagnosticsProvider.getDiagnostics();

        // Assert
        expect(diagnostics[0].message).to.equal('The address is not a valid Bech32m address.');
    });
    ...
    test('More than two specifiers is invalid.', () => {
        ...
        // Assert
        expect(diagnostics[0].message).to.equal('The address contains multiple specifier separators.');
    });
    ...
    test('One specifier is invalid.', () => {
        ...
        // Assert
        expect(diagnostics[0].message).to.equal('The address does not contain a network and entity specifiers.');
    });
    ...
    test('Type and entity specifier mismatch should fail.', () => {
        ...
        // Assert
        expect(diagnostics[0].message).to.equal('Invalid ComponentAddress: A ComponentAddress can not have an HRP beginning in "package"');
    });
    ...
    test('Inconsistent network specifiers should cause error.', () => {
        ...
        // Assert
        expect(diagnostics[0].message).to.contain('Inconsistent Network Specifier');
    });
