    ...
    enterManifestInstruction(context: ManifestInstructionContext) {
        ...
        if (end) {
            ...
            if (children) {
                ...
                switch (instruction) {
                    case 'CLONE_PROOF':
                        let proofDefinition: ProofDefinition = proofDefs.splice(0, 1)[0];
                        let proofExists: boolean = this.availableProofs.map((x) => x.proof.toString()).indexOf(proofDefinition.proof.toString()) !== -1;
                        if (!proofExists) {
                            diagnostics.push(new Diagnostic(proofDefinition.range, "Invalid Proof: No Proof with the given name of Id exists. This either means that the Proof has been moved as a result of a previous instruction or that it never existed.", DiagnosticSeverity.Error));
                        }
                        break;
                    case 'CREATE_PROOF_FROM_BUCKET':
                        let bucketDefinition: BucketDefinition = bucketDefs.splice(0, 1)[0];
                        let bucketExists: boolean = this.availableBuckets.map((x) => x.bucket.toString()).indexOf(bucketDefinition.bucket.toString()) !== -1;
                        if (!bucketExists) {
                            diagnostics.push(new Diagnostic(bucketDefinition.range, "Invalid Bucket: No Bucket with the given name of Id exists. This either means that the Proof has been moved as a result of a previous instruction or that it never existed.", DiagnosticSeverity.Error));
                        }
                        break;
                }
