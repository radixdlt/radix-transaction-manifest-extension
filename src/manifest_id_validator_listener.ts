...
class Bucket {
	...
	static fromBucketString(bucketString: string): Bucket | undefined {
		let matches: RegExpMatchArray | null = bucketString.match(
			/Bucket\((\".*\"|\d+u32)\)/
		);
		if (matches) {
			...
			if (matchedString.startsWith('"') && matchedString.endsWith('"')) {
				...
				let bucketId: number = parseInt(
					matchedString.replace("u32", "")
				);
...
class Proof {
	...
	static fromProofString(proofString: string): Proof | undefined {
		let matches: RegExpMatchArray | null = proofString.match(
			/Proof\((\".*\"|\d+u32)\)/
		);
		if (matches) {
			let matchedString: string = matches[1];
...
export class ManifestIdValidator implements TransactionManifestListener {
			...
			if (children) {
				...
				for (var token of flattenedChildren) {
					if (token instanceof BucketContext) {
						let bucketDefinition: BucketDefinition | undefined =
							this.getBucketDefinition(token);
						if (bucketDefinition) {
							bucketDefs.push(bucketDefinition);
						}
					} else if (token instanceof ProofContext) {
						let proofDefinition: ProofDefinition | undefined =
							this.getProofDefinition(token);
						if (proofDefinition) {
				...
				switch (instruction) {
					case "CLONE_PROOF":
						let proofDefinition: ProofDefinition = proofDefs.splice(
							0,
							1
						)[0];
						let proofExists: boolean =
							this.availableProofs
								.map((x) => x.proof.toString())
								.indexOf(proofDefinition.proof.toString()) !==
							-1;
						if (!proofExists) {
							diagnostics.push(
								new Diagnostic(
									proofDefinition.range,
									"Invalid Proof: No Proof with the given name of Id exists. This either means that the Proof has been moved as a result of a previous instruction or that it never existed.",
						...
						break;
					case "CREATE_PROOF_FROM_BUCKET":
						let bucketDefinition: BucketDefinition =
							bucketDefs.splice(0, 1)[0];
						let bucketExists: boolean =
							this.availableBuckets
								.map((x) => x.bucket.toString())
								.indexOf(bucketDefinition.bucket.toString()) !==
							-1;
						if (!bucketExists) {
							diagnostics.push(
								new Diagnostic(
									bucketDefinition.range,
									"Invalid Bucket: No Bucket with the given name of Id exists. This either means that the Proof has been moved as a result of a previous instruction or that it never existed.",
									DiagnosticSeverity.Error
								)
							);
						}
						...
						for (var bucketDef of bucketDefs) {
							if (!this.addBucketDefinition(bucketDef)) {
								diagnostics.push(
									new Diagnostic(
										bucketDef.range,
										"Invalid Bucket Name or Id: A Bucket was previously created with the same name or Id. To resolve this issue, consider changing the name of the Bucket to a unique name. ",
										DiagnosticSeverity.Error
									)
								);
							}
						...
						for (var proofDef of proofDefs) {
							if (!this.addProofDefinition(proofDef)) {
								diagnostics.push(
									new Diagnostic(
										proofDef.range,
										"Invalid Proof Name or Id: A Proof was previously created with the same name or Id. To resolve this issue, consider changing the name of the Proof to a unique name. ",
										DiagnosticSeverity.Error
									)
								);
							}
						...
						for (var bucketDef of bucketDefs) {
							if (!this.removeBucket(bucketDef.bucket)) {
								diagnostics.push(
									new Diagnostic(
										bucketDef.range,
										"Invalid Bucket: No Bucket with the given name of Id exists. This either means that the Bucket has been moved as a result of a previous instruction or that it never existed.",
										DiagnosticSeverity.Error
									)
								);
							}
	...
	private addBucketDefinition = (
		bucketDefinition: BucketDefinition
	): boolean => {
		let bucketDefIndex: number = this.availableBuckets
			.map((x) => x.bucket.toString())
			.indexOf(bucketDefinition.bucket.toString());
		if (bucketDefIndex === -1) {
	...
	private addProofDefinition = (
		proofDefinition: ProofDefinition
	): boolean => {
		let proofDefIndex: number = this.availableProofs
			.map((x) => x.proof.toString())
			.indexOf(proofDefinition.proof.toString());
		if (proofDefIndex === -1) {
	...
	getDiagnostics = (): Diagnostic[] => {
		this.diagnostics.push(
			...this.availableBuckets.map(
				(x) =>
					new Diagnostic(
						x.range,
						"Dangling Bucket: This Bucket nas not been deposited anywhere during this transaction and will therefore lead the transaction to fail"
					)
			)
		);
