...
export async function activate(context: vscode.ExtensionContext) {
	...
	const updateDefaultAccountAddress = async () => {
		let address: string | undefined =
			await resimUtils.getDefaultAccountAddress();
	...
	const updateDefaultAccountButtonText = (
	...
	const run = vscode.commands.registerCommand("extension.runManifest", () => {
		...
		if (filePath === undefined) {
			...
			let terminal: vscode.Terminal =
				extensionUtils.getTerminal("Run Manifest");
			terminal.show(true);
	...
	const defaultAccount = vscode.commands.registerCommand(
		...
		async () => {
			...
			let currentDefaultAccount: string | undefined =
				await updateDefaultAccountAddress();
	...
	let newAccountButton: vscode.StatusBarItem =
		vscode.window.createStatusBarItem(1, 0);
	...
	vscode.languages.registerDocumentFormattingEditProvider("rtm", {
		provideDocumentFormattingEdits(
			...
			for (var range of ranges) {
				...
				let formattedInstruction: string = instruction
					.split(/[\s|\n|\r|\t]/)
