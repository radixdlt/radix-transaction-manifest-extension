 ...
 */
export default class IdValidationDiagnosticsProvider extends DiagnosticsProvider implements TransactionManifestListener {
    ...
    enterTakeFromWorktop(context: TakeFromWorktopContext) {
        this.addNewObject(context.children![context.children!.length - 2] as ScryptoObjectContext);
    }
    ...
    enterReturnToWorktop(context: ReturnToWorktopContext) {
        this.removeObject(context.children![context.children!.length - 2] as ScryptoObjectContext);
    }
    ...
    enterCreateProofFromAuthZoneByAmount(context: CreateProofFromAuthZoneByAmountContext) {
        this.addNewObject(context.children![context.children!.length - 2] as ScryptoObjectContext);
    }
    
    enterCreateProofFromAuthZoneByIds(context: CreateProofFromAuthZoneByIdsContext) {
        this.addNewObject(context.children![context.children!.length - 2] as ScryptoObjectContext);
    }
    
    enterCreateProofFromBucket(context: CreateProofFromBucketContext) {
        let bucketContext: BucketContext = context.children![context.children!.length - 3] as BucketContext;
        let proofContext: ProofContext = context.children![context.children!.length - 2] as ProofContext;
        this.ensureExistence(bucketContext);
        this.addNewObject(proofContext);
    }
    
    enterCloneProof(context: CloneProofContext) {
        let originalProofContext: ProofContext = context.children![context.children!.length - 3] as ProofContext;
        let secondProofContext: ProofContext = context.children![context.children!.length - 2] as ProofContext;
    ...
    private getProofs(): ProofContext[] {
        return this.scryptoObjectDefinitions.filter((x) => x[0].getType() === ScryptoObjectType.proof).map((x) => x[1] as ProofContext);
    }
    ...
    private getBuckets(): BucketContext[] {
        return this.scryptoObjectDefinitions.filter((x) => x[0].getType() === ScryptoObjectType.bucket).map((x) => x[1] as BucketContext);
    }
    ...
    private removeObject(context: ScryptoObjectContext) {
        ...
        let index: number = this.scryptoObjectDefinitions.map((x) => x[0].isEqual(currentObject)).indexOf(true);
        if (index === -1) {
     ...
     */
    private handleCallMethodOrFunction(context: CallMethodContext | CallFunctionContext) {
        ...
        for (var child of flattenedChildren) {
            if (child instanceof BucketContext || child instanceof ProofContext) {
